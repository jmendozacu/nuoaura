...
class Mage_Checkout_MultishippingController extends Mage_Checkout_Controller_Action
{
	...
	/**
	 * Tapan add this method to upload document through Ajax
	 */
    ...
    public function overviewPostAction()
    {
        ...
        try {
			...
			->from('sales_flat_order_payment', array('MAX(entity_id) as last_entity_id'));
			$rowArray =$connection->fetchRow($select);   //return row			
			$last_entity_id = $rowArray['last_entity_id'];
			/********************** Tapan ***************/
			
            $this->_getCheckout()->createOrders();
			/**
			 * Tapan add this for update file upload in payment table
			 */
			...
			$where = $connection->quoteInto('entity_id > ?', $last_entity_id);
