...
class ZetaPrints_OrderApproval_CustomerCartController
  ...
  public function editAction () {
   ...
   /*
     ...
     $displayQuote= Mage::getModel('sales/quote');
     $itemCollection= $quote->getItemsCollection();
      foreach($itemCollection as $item){

           if('Pending approval'==Mage::helper('orderapproval')->getNoticeFromItem($item)){
           $displayQuote->addItem($item);
           
           }
      }
      
      $displayQuote->collectTotals();
      $displayQuote->reserveOrderId();
      $displayQuote->setIsActive(0);
      $displayQuote->save();
  ...
  public function massUpdateApprovalStateAction () {
    ...
    $approverQuote = Mage::getModel('checkout/cart')->getQuote();

    foreach ($item_ids as $item_id) {
      $item = Mage::getModel('sales/quote_item')->load($item_id);

      // add products to approver cart
       ...
       if($state == ZetaPrints_OrderApproval_Helper_Data::APPROVED){
          ...
          $approverCart->getQuote()->setTotalsCollectedFlag(false);
          $approverCart->save();
         //$product = Mage::getModel('catalog/product')->load($item_id);
         
        // print_r($item);echo "<br><br>";
       
         //print_r(get_class_methods($approverCart));die();
         
         //$approverQuote->addOrderItem($item,$qty);
         }
